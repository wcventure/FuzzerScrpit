...
def main(argv):
    ...
    for eachfile in fileList:
        with open (TargetFolder + eachfile, 'r') as f:
            ...
            for line in f.readlines():
                ...
                if 'unique_failAlloc' in line:
                    left,right = line.split(':',1)
                    UniqueFailToAlloc.append(right.strip())
                if 'total_failAlloc' in line:
                    left,right = line.split(':',1)
    ...
    with open ('./tmp.txt', 'w+') as fi:
        ...
        if not UniqueFailToAlloc == []:
            fi.write('U_Fail_A:\n')
            for each in UniqueFailToAlloc:
                fi.write(str(each)+'\t')
            fi.write('\n')
        if not TotalFailToAlloc == []:
            fi.write('T_Fail_A:\n')
            for each in TotalFailToAlloc:
                fi.write(str(each)+'\t')
            fi.write('\n')
